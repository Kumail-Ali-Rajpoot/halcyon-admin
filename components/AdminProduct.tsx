...
function AdminProduct({ product }:Props) {
  ...
  return (
    <div
      key={product.id}
      className="bg-gray-800 border text-amber-50 border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all p-4 flex flex-col lg:flex-row gap-4"
    >
<img
...
/>
      <div className="flex flex-col justify-between flex-grow min-w-0">
        <div>
          ...
          <div className="text-sm flex flex-wrap gap-3.5 justify-around space-y-1 mb-3">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          <button onClick={handleEdit} className="bg-blue-600 flex gap-1.5 cursor-pointer text-white text-sm px-4 py-1.5 rounded-md hover:bg-blue-700 transition">
            <Edit size={20}></Edit> Edit
          </button>
        <form action={deleteProduct}>
          ...
          <button className="bg-gray-600 cursor-pointer text-white text-sm px-4 py-1.5 rounded-md hover:bg-gray-700 transition">
            🔍 View
          </button>
        </div>
      </div>
